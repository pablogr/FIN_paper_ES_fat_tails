...
from module_fitting import first_iteration, first_iteration_single_param, calc_loss_stable
...
def find_starting_point( dataset_in, consider_skewness = True, res_nct={'nct_loc':float('NaN')}, verbose=0):
    ...
    if (verbose>0): print("   - The skewness of the time series is:",skewness_dataset,"\n")
...
def fit_to_stable_global_minimum( dataset_in,  n_random_tries,  max_n_iter, consider_skewness=True, verbose=0, res_nct=None ):
    ...
    if (len(dataset_in <= 50)): n_random_tries *= 2

    li_locations_sg, li_scalings_sg, li_b_sg, li_a_sg = find_starting_point( dataset_in, consider_skewness, res_nct, verbose )
        ...
        for sca_param_tt in li_scalings_sg:
            for beta_param_tt in li_b_sg:
                for alpha_param_tt in li_a_sg:
                      if (verbose>1): print("Params IN", loc_param_tt, sca_param_tt, beta_param_tt, alpha_param_tt ) #if (verbose>0):
                      for trial_counter in range(n_random_tries):

                          loss_fnd, loc_param_fnd, sca_param_fnd, beta_param, alpha_param_fnd  = fit_to_levy_stable_local_minimum( dataset_in, max_n_iter, consider_skewness, lim_params, loc_param_tt, sca_param_tt, beta_param_tt, alpha_param_tt, trial_counter, verbose )
                          ...
                          collect()
                          if (verbose>1): print("Params IN",  loc_param_tt, sca_param_tt, beta_param_tt, alpha_param_tt , "; Params OUT", loc_param_fnd, sca_param_fnd, beta_param, alpha_param_fnd, "; LOSS:", loss_fnd )
...
def fit_to_levy_stable_local_minimum( dataset_in, max_n_iter, consider_skewness, lim_params, loc_param0, sca_param0, beta_param0, alpha_param0, trial_counter, verbose=0  ):
    ''' This function finds the generalized hyperbolic distribution which best{ fits to the input dataset.
    ...
    while ( (n_iter < max_n_iter/2) or ( ( (( abs(loss1 - loss0) > tolerance_fitting) or (metropolis > 0.9) ) or (loss1 == 9999) ) and (n_iter < max_n_iter) )):
        ...
        try:
            ...
            loss1 = calc_loss_stable(dataset_in, loc_param1, sca_param1, beta_param1, alpha_param1)
            if ( isnan(loss1)):
        ...
        if (verbose > 1): print("Niter=",n_iter,") Params:", loc_param1, sca_param1, beta_param1, alpha_param1,"; loss=", loss1)

        ''' 
        if  ( (trial_counter==1) and (n_iter%3)==0): # DEV: This block apparently worsens the output
...
def fit_to_levy_stable_local_minimum_sweep_single_param( sp, dataset_in, consider_skewness, lim_params, loc_param0, sca_param0, beta_param0, alpha_param0, verbose ):
    ...
    print("Saliendo tenemos: Loss=",loss_opt,"; Params:", loc_param_opt, sca_param_opt, beta_param_opt, alpha_param_opt)#xxx
...
def fit_to_levy_stable_local_minimum_sweep_beta( dataset_in, lim_params, loc_param0, sca_param0, beta_param0, alpha_param0, loss0  ):
    ...
    for i in range(n_essais):
        ...
        loss1 = calc_loss_stable(dataset_in, loc_param0, sca_param0, my_beta, alpha_param0)
...
def calculate_gradient_params( dataset_in, consider_skewness, loc_param, sca_param, beta_param, alpha_param):
    ...
    h0 = sca_param / 20000 #old   sca_param / 10000
    h1 = beta_param / 500 + 1 / 10000 #old   beta_param / 100 + 1 / 1000
    h2 = alpha_param / 1000 # old alpha_param / 100
...
def calculate_gradient_single_param( sp, dataset_in, consider_skewness, lim_params, loc_param, sca_param, beta_param, alpha_param):
    ...
    # DEV: This gives errors if there are zeros in dataset_in_pdf:
    # grad_single = -np.sum( grad_single /dataset_in_pdf  )  / len(dataset_in)

    grad_single_out = 0.0; count1=0
    for i in range(len(dataset_in_pdf)):
        if (abs(dataset_in_pdf[i]) > 1.0e-12):
            grad_single_out += grad_single[i] / dataset_in_pdf[i]
            count1 += 1
    if (count1>0):
        grad_single_out = - grad_single_out / count1
    else:
        grad_single_out = cauchy.rvs(size=1, loc=0, scale=0.001)[0]
        print("WARNING: Unable to find a gradient; random value ("+grad_single_out+") for it was generated.")

    del dataset_in_pdf; del sp; del dataset_in; del loc_param; del sca_param; del beta_param; del alpha_param; del grad_single

    return grad_single_out
