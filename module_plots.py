...
dict_my_colors = {"norm-ES":mcolors.CSS4_COLORS["orangered"],
                      "norm-Avg":mcolors.CSS4_COLORS["firebrick"],
                      "nct-ES":mcolors.CSS4_COLORS["dodgerblue"],
                      "nct-Avg":mcolors.CSS4_COLORS["steelblue"],
                      "genhyperbolic-ES":"#ff1493",
                      "genhyperbolic-Avg":mcolors.CSS4_COLORS["darkmagenta"],
                      "levy_stable-ES":'#ffa500',
                      "levy_stable-Avg":mcolors.CSS4_COLORS["goldenrod"]}

dict_distribs = {"norm":"Normal", "nct":"t-student", "genhyperbolic":"Gen. hyperb.", "levy_stable": "Levy stable"}
...
def make_plots_2D( quantity, output_dir, filenameout, x, li_y, my_labels=["<ES>","ES+/-2 sigma_ES",None,"sigma_ES"], my_colors=["black","blue","blue","red"], my_linestyles= ["-",":",":","-."], my_title="", color_fill='C0', prefix="", ylim=None, narrow=False, my_loc_label=None, my_markershapes=["o","o","o","o"] ):
    '''This function makes e.g. the plots of all the spreads.'''
    ...
    if (narrow):
        ...
        #plt.legend(frameon = 1).get_frame().frame.set_linewidth(0)#set_edgecolor('white')
    ...
    if (abs(li_y[0][-1])<0.01):
        legend_loc = "upper right"
    else:
        legend_loc = "upper right"
    if ( ("mean" in filenameout) and ("_distrib_" in filenameout) ):
        legend_loc = "center right"
    if (my_loc_label!=None):
        legend_loc = my_loc_label

    #legend_loc = "lower right" #xxx


    for (y,my_linst,my_col,my_lab,my_markershape) in zip(li_y,my_linestyles,my_colors,my_labels,my_markershapes):
        if (my_col=="gray"):
            ax.plot( [x[0]-100,x[-1]+100], [y[0],y[-1]],  linestyle="-",  label=my_lab, color=my_col, linewidth=my_lw )
        else:
            if ( (my_lab!="") ):
                ax.plot( x, y, my_markershape, linestyle=my_linst, label=my_lab, color=my_col, linewidth=my_lw, markersize=my_ms)
            else:
                ax.plot(x, y, my_markershape, linestyle=my_linst, color=my_col, linewidth=my_lw, markersize=my_ms)


    if ( (len(li_y)>=3) and (my_colors[0]!="gray") ) :
        ax.fill_between(  x, li_y[1], li_y[2], alpha=0.2, color=color_fill); #ax.set_title('fill between y1 and 0')
    #ax.fill_between(x, y_est - y_err, y_est + y_err, alpha=0.2)


    plt.legend(loc=legend_loc, labelspacing=0.5, handlelength=4) #bbox_to_anchor=(0.5, 0.1, 0.5, 0.5),
    fig.tight_layout()
    if(my_title!=""): ax.set_title(my_title,fontsize=my_fs)
    ax.set_xlabel(r'$N_{sample}$', fontsize=my_fs)
    if (quantity == "ES"): ax.set_ylabel(r'$  ES  $', fontsize=my_fs)
    elif (quantity == "Avg"): ax.set_ylabel(r'$  Avg  $', fontsize=my_fs)
    else: print("WARNING: No label found")
    plt.xlim([min(min(x) - 1, 0), max(x) + 1])
    if (ylim != None):
        plt.ylim([ylim[0], ceil( 40* (ylim[1]) )/40 ] )

    #plt.ylim([-0.04,0.14]); plt.yticks([  -0.04, -0.02, 0.00, 0.02, 0.04, 0.06, 0.08, 0.10, 0.12, 0.14],fontsize=14)  # xxx
    plt.yticks( fontsize=14) #  plt.yticks([  1.6,1.7,1.8,1.9,2.0],fontsize=14)

    filepath_plot = path.join(output_dir,"Plots", filenameout)
    plt.savefig(filepath_plot, format="pdf", bbox_inches="tight")
...
def plot_histogram(  fitted_time_series, plot_several_curves=False, params_several_curves=None ):
    ...
    # Define the bins
    len_to_remove = int(0.01 * len(fitted_time_series.np_ret))
    lim_bins =  0.85 #4*np.std( fitted_time_series.np_ret[len_to_remove:-len_to_remove] )
    num_bins = 17 #11
    bins = [-lim_bins  ] # OLD: [-lim_bins + loc_in ]; Si el loc se calculó mal, esto hace que el histograma tb se pinte mal.
    labels = []
    for i in range(1,num_bins+1):
        bins   += [ -lim_bins + i*0.1] #[ -lim_bins + i*(2*lim_bins)/num_bins ]
        if (i%2 == 0):
          if (lim_bins >= 1):
    ...
    counts0, bins = np.histogram( fitted_time_series.np_ret, bins ) #old: ts_to_fit
    counts = np.zeros(len(counts0))
    for i in range(len(counts0)):
        counts[i] = counts0[i]/len(fitted_time_series.np_ret) #old: ts_to_fit

    # Plot the histogram
    x = np.array( [ (bins[i]+bins[i+1])/2 for i in range(len(labels)) ] )

    width = 0.91*(bins[i+1]-bins[i])
    fig, ax = plt.subplots()
    ax.bar(x , counts, width, label='Histogram (normalized)', color=mcolors.CSS4_COLORS["midnightblue"]) # "darkblue" 'mediumblue'
    ...
    for fitting_function in list_sweep:
        ...
        x_cont = x
        x_cont0 = np.array( [ x[0] + i*(x[-1]-x[0])/(60*num_bins-1) for i in range(60*num_bins) ]  ) #np.linspace(0, len(labels) - 1, num=num_bins*10  )
        ...
        if ( fitting_function =="norm"):
           ...
           mycolor  = mcolors.CSS4_COLORS["orangered"]#"red"
           ...
           mycolor = mcolors.CSS4_COLORS["dodgerblue"] #"lime"#'#00ced1'#
           ax.plot(x_cont0, y_cont0, color='black', lw=5.2)
           if not (plot_several_curves): ax.plot(x_cont, y_cont, '.', color='black', markersize=17)
        ...
        del x_cont; del x_cont0; del y_cont0; del scale_in; del skew_in; del tail1_in; del tail2_in; del mycolor;

    # Add labels to the plot:
    if not (plot_several_curves):
        plt.ylim([0,  max( (max(counts) *1.3), ((max(y_cont))*1.3) ) ] )
    else:
        plt.ylim([0, max( max(counts) * 1.5, (max(y_cont))*1.3) ])
    #plt.ylim([0,0.16])
    ax.set_ylabel('Probability',fontsize=15)

    dict_xlabel = { "absolute":"abs. ret.", "relative":"rel. ret.", "logarithmic":"log. ret." }
    ax.set_xlabel( dict_xlabel[fitted_time_series.type_of_return],fontsize=14)
    ...
    del len_to_remove; del lim_bins; del num_bins; del labels; del x; del width; del counts; del counts0; del y_cont;
...
#---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# bbb
def plot_bare_histograms( input_params, filename, quantity):
    '''This function makes the histogram plots without actual fitting'''

    if not (input_params.make_plots): return

    filepathin = path.join( input_params.output_directory, "Results", filename)

    list_hist = list(pd.read_csv(filepathin, header=0, nrows=1))[1:]

    for colname in list_hist:
        prod_name = colname.split("/")[0]
        distrib_type = colname.split("/")[1]
        size = str(colname.split("/")[3]).replace("size", "")
        print(" - Now plotting the histogram of",prod_name,"-",distrib_type, "with",size,"sample points.")
        my_title = prod_name + "\n" + dict_distribs[distrib_type] +" distribution"+ "\nSample size = "+size
        df0 = pd.read_csv(filepathin, header=0, usecols=[colname])
        filepath_plot = path.join(input_params.output_directory, "Plots","Histograms", "Histogram_"+colname.replace("/","_")+".pdf" )
        if (quantity=="ES"):
            if ("AAPL" in filename):
                low_lim=0; hi_lim=0.2
                if (distrib_type=="norm"): hi_lim=0.2
                elif (distrib_type == "nct"): hi_lim = 0.2
                elif (distrib_type == "levy_stable"): hi_lim = 0.4
            elif ("SHELL" in filename):
                low_lim=0; hi_lim=0.2
                if (distrib_type=="norm"): hi_lim=0.2
                elif (distrib_type == "nct"): hi_lim = 0.2
                elif (distrib_type == "levy_stable"): hi_lim = 0.4
            elif ("BASF" in filename): # Basf bond
                low_lim=0; hi_lim=2
                if (distrib_type=="norm"): hi_lim=1
                elif (distrib_type == "nct"): hi_lim = 8
                elif (distrib_type == "levy_stable"): hi_lim = 10
            elif ("CharlesSchwab" in filename): # Charles Schwab bond
                low_lim=0; hi_lim=8
                if (distrib_type=="norm"): hi_lim=1
                elif (distrib_type == "nct"): hi_lim = 8
                elif (distrib_type == "levy_stable"): hi_lim = 10
            ...
            if ("AAPL" in filename):
                if (distrib_type == "norm"): hi_lim = 0.01
                elif (distrib_type == "nct"): hi_lim = 0.01
                elif (distrib_type == "genhyperbolic"): hi_lim = 0.01
                elif (distrib_type == "levy_stable"): hi_lim = 0.02
            elif ("SHELL" in filename):
                if (distrib_type == "norm"): hi_lim = 0.01
                elif (distrib_type == "nct"): hi_lim = 0.01
                elif (distrib_type == "genhyperbolic"): hi_lim = 0.01
                ...
                if (distrib_type == "norm"): hi_lim = 0.12
                elif (distrib_type == "nct"): hi_lim = 0.4
                elif (distrib_type == "genhyperbolic"): hi_lim = 0.12
                elif (distrib_type == "levy_stable"): hi_lim = 0.4
            elif ("CharlesSchwab" in filename): # Charles Schwab bond
                if (distrib_type == "norm"): hi_lim = 0.3
                elif (distrib_type == "nct"): hi_lim = 0.3
                elif (distrib_type == "genhyperbolic"): hi_lim = 0.3
                elif (distrib_type == "levy_stable"): hi_lim = 0.4
            else:
                if (distrib_type == "norm"): hi_lim = 0.12
                elif (distrib_type == "nct"): hi_lim = 0.4
            ...
            low_lim = -abs(hi_lim)
        array_to_plot = [ x for x in df0[colname].to_numpy() if x!=None ]
        plot_one_histogram( quantity, distrib_type, filepath_plot, array_to_plot, hi_lim, low_lim, 100, my_title )

    return

# ------------------------------------------------------------------------------------------------

def plot_one_histogram( quantity, distrib_type, filepath_plot, array_in, truncation_limit, lim_low=0, num_bins=40, my_title="" ):

    # Define the bins
    assert quantity in ["ES","Avg"]
    if (quantity=="Avg"):truncation_limit=abs(lim_low)
    bins = [ lim_low  ]
    labels = []
    for i in range(1,num_bins+1):
        bins   += [ lim_low + i*(truncation_limit-lim_low)/num_bins ]
        if (i%2 == 0):
          if (truncation_limit >= 1):
            labels += [ "{:.4f}".format( lim_low + (i-0.5)*(2*truncation_limit)/num_bins  ) ] # old: labels += [ "{:.4f}".format( -lim_bins + loc_in  + (i-0.5)*(2*lim_bins)/num_bins  ) ]
          else:
            labels += ["{:.5f}".format( lim_low  + (i - 0.5) * (2 * truncation_limit) / num_bins)] # old:  ["{:.5f}".format(-lim_bins + loc_in  + (i - 0.5) * (2 * lim_bins) / num_bins)]
        else:
            labels += [None]
    ...
    width = 0.91*(bins[i+1]-bins[i])
    fig, ax = plt.subplots()
    ax.bar(x , counts, width, label='Histogram (normalized)', color=dict_my_colors[distrib_type+"-"+quantity])

    if (quantity=="ES"): ax.set_xlabel(r'$  ES  $', fontsize=20)
    else:  ax.set_xlabel(r'  Avg  ', fontsize=20)
    ax.set_ylabel("Probability", fontsize=20)
    if (my_title != ""): ax.set_title( my_title,fontsize=22 )

    plt.xticks(rotation=45, ha="right",fontsize=14)
    plt.yticks( fontsize=14)

    plt.savefig(filepath_plot.replace(".pdf","-"+quantity+".pdf"), format="pdf", bbox_inches="tight")
    plt.clf(); plt.cla(); plt.close('all')
...
def write_title1( quantity, filename, distrib ):

    for keyword in ["ES_distrib_cont","ES_distrib_discr","discr_comp_ES_","mean_ES","median_ES"]:
        filename = filename.replace(keyword,"")
    if (quantity == "ES"):
        my_title = r'$ ES $' + " - " + filename.split("-")[1]
    elif (quantity == "Avg"):
        my_title = r'$ Avg $' + " - " + filename.split("-")[1]
    else:
        my_title = ""
    for keyword in ["ES_distrib_cont","ES_distrib_discr","ES_HS_vs_N", "ES_vs_N", "Avg_vs_N","Avg_HS_vs_N", "_mean", "_median", "_norm", "_nct", "_genhyperbolic", "_levy_stable", "_absolute", "_relative", "_logarithmic", ".pdf", ".png", ".csv"]:
        my_title = my_title.replace( keyword, "")
...
# -----------------------------------------------------------------------------------------------
#yyy
def plot_comparison( output_dir, filename1, filename2, label1="", label2="",my_title="" ):
    '''This function plots a comparison of values of mean, median and standard deviation of the ES for the two input files.'''

    df1 = pd.read_csv( path.join(output_dir,"Results",filename1))
    df2 = pd.read_csv(path.join(output_dir, "Results", filename2))
    x = df1["N"].to_numpy();  x2 = df2["N"].to_numpy(); assert (x.all()==x2.all())
    list_colors1 = [ mcolors.CSS4_COLORS["navy"], mcolors.CSS4_COLORS["darkgreen"],  mcolors.CSS4_COLORS["orangered"] ]
    list_colors2 = [mcolors.CSS4_COLORS["mediumblue"], mcolors.CSS4_COLORS["limegreen"], mcolors.CSS4_COLORS["darkred"]]
    list_ylabels = [r'$  \langle ES  \rangle $', r'median $  ES  $', r'$\sigma_{ES} $' ]

    for quantity,my_ylabel,my_color1,my_color2 in zip(["<ES>","median_ES","stdev_ES"],list_ylabels,list_colors1,list_colors2):

        fig, ax = plt.subplots()

        y1 = df1[quantity].to_numpy()
        y2 = df2[quantity].to_numpy()

        ax.plot(x, y1, "o", linestyle="-",  color=my_color1, markersize=4,label=label1)#, my_markershape, linestyle=my_linst, label=my_lab, color=my_col, linewidth=my_lw, markersize=my_ms)
        ax.plot(x, y2, "s", linestyle="--", color=my_color2,  markersize=4,label=label2)

        legend_loc = "center right"

        plt.legend(loc=legend_loc, labelspacing=0.5, handlelength=4) #bbox_to_anchor=(0.5, 0.1, 0.5, 0.5),
        fig.tight_layout()
        ax.set_xlabel(r'$N_{sample}$', fontsize=14)
        ax.set_ylabel(my_ylabel, fontsize=14)

        if(my_title!=""): ax.set_title(my_title,fontsize=18)
        plt.xlim([min(min(x) - 1, 0), max(x) + 1])
        ylim = [min(min(y1),min(y2)), max(max(y1),max(y2))]
        plt.ylim([ floor(200 * (ylim[0])) / 200  , ceil(200 * (ylim[1])) / 200 ])

        #plt.ylim([-0.04,0.14]); plt.yticks([  -0.04, -0.02, 0.00, 0.02, 0.04, 0.06, 0.08, 0.10, 0.12, 0.14],fontsize=14)  # xxx
        #  plt.yticks([  1.6,1.7,1.8,1.9,2.0],fontsize=14)

        plt.xticks(fontsize=12);plt.yticks(fontsize=11)
        filepath_plot = path.join(output_dir,"Plots", filename1.replace(".csv","")+"-"+filename2.replace(".csv","")+"_merged-"+quantity+".pdf")
        plt.savefig(filepath_plot, format="pdf", bbox_inches="tight")
...
def plot_vs_N( quantity, output_dir, filename_in, prefix="", ylim=None ):
    '''This function makes the plots of the average or ES vs the number of points of the sample size.
    It plots its mean, median, standard deviaton and 95% confidence interval.'''

    assert quantity in ["Avg","ES"]

    df0 = pd.read_csv( path.join(output_dir,"Results",filename_in), header=0)
    if (df0.empty):
        print("\nERROR: The dataframe of file "+str(path.join(output_dir,"Results",filename_in))+"\nis empty. The plot-vs-N could not be done.\n")
        return
    index_name =  list(df0)[0]
    li_plots = pd.DataFrame((pd.DataFrame( df0[index_name] )).drop_duplicates())
    li_plots = li_plots[index_name].values.tolist()
    df0.set_index(index_name,inplace=True)
    ci_factor = 2.0

    for ind_label in li_plots:

        df1 = df0.loc[ind_label]
        x =  df1["N"].to_numpy()
        y1 = df1["<"+quantity+">"].to_numpy()
        y4 = df1["stdev_"+quantity].to_numpy()
        y2 = y1 - ci_factor * y4
        y3 = y1 + ci_factor * y4
        li_y_to_plot = [ y1, y2, y3, y4]
        filename = prefix+"_mean-"+ind_label.replace("/","_")+".pdf"

        my_title = write_title1( quantity, filename, dict_distribs[ind_label.split("/")[1]] )

        col1 = mcolors.CSS4_COLORS["navy"]; col2 = mcolors.CSS4_COLORS["mediumblue"];  col3 = mcolors.CSS4_COLORS["orangered"]
        my_linestyles = ["-", ":", ":", "-."]

        ''' 
        if ("_vs_N_HS"  in prefix): # mode "ES_HS-vs-N"
            col1 = mcolors.CSS4_COLORS["navy"]; col2 = mcolors.CSS4_COLORS["mediumblue"];  col3 = mcolors.CSS4_COLORS["orangered"]
            my_linestyles = [":", ":", ":", "-."]
        else:   # mode  "ES_distrib-vs-N"
            col1 = "gray";   col2 = mcolors.CSS4_COLORS["dodgerblue"];  col3 = mcolors.CSS4_COLORS["crimson"]
            my_linestyles = ["-", ":", ":", "-."]
        '''


        if quantity=="ES":      my_labels = [r'$ \langle ES \rangle $', r'$ \langle ES \rangle \pm 2 \sigma_{ES}$', None, r'$\sigma_{ES}$']
        elif quantity == "Avg":  my_labels = [r'$ \langle Avg \rangle $', r'$ \langle Avg \rangle \pm 2 \sigma_{Avg}$', None, r'$\sigma_{Avg}$']
        color_fill = dict_my_colors[ind_label.split("/")[1] + "-" + quantity]
        make_plots_2D( quantity, output_dir, filename, x, li_y_to_plot, my_labels, [col1,col2,col2,col3], my_linestyles, my_title, color_fill, prefix, ylim )
    ...
    for ind_label in li_plots:
        ...
        df1 = df0.loc[ind_label]
        x = df1["N"].to_numpy()
        y1 = df1["median_"+quantity].to_numpy()
        y2 = df1["CI_1"].to_numpy()
        y3 = df1["CI_2"].to_numpy()
        li_y_to_plot = [y1, y2, y3 ]
        filename = prefix+"_median-" + ind_label.replace("/", "_") + ".pdf"
        color_fill = dict_my_colors[ind_label.split("/")[1] + "-" + quantity]
        col1 = mcolors.CSS4_COLORS["navy"]; col2 = mcolors.CSS4_COLORS["mediumblue"] # DEV: See https://matplotlib.org/stable/gallery/color/named_colors.html
        make_plots_2D( quantity, output_dir, filename, x, li_y_to_plot, ["Median", "95% confidence interval", None ],[col1, col2, col2], ["-", ":", ":" ], my_title, color_fill, prefix, ylim)

    del df0; del filename; del x; del li_y_to_plot

    return

# -----------------------------------------------------------------------------------------------

def plot_discr_cont_vs_N(input_params, quantity, filename_in1, filename_in2, prefix1="", prefix2=""):
    assert quantity in ["Avg", "ES"]
    from module_ES_vs_N import calc_ES_whole_dataset

    output_dir = input_params.output_directory

    df1 = pd.read_csv(path.join(output_dir, "Results", filename_in1), header=0,
                      usecols=["product/distrib/ret", "N", "<" + quantity + ">", "median_" + quantity])
    df2 = pd.read_csv(path.join(output_dir, "Results", filename_in2), header=0,
                      usecols=["product/distrib/ret", "N", "<" + quantity + ">", "median_" + quantity])
    df1.set_index("product/distrib/ret", inplace=True)
    df2.set_index("product/distrib/ret", inplace=True)
    li_plots = df1.index.drop_duplicates().values.tolist()
    li_index_2 = df2.index.drop_duplicates().values.tolist()
    assert li_plots == li_index_2

    for ind_label in li_plots:

        ES_cont_whole_dataset = calc_ES_whole_dataset(input_params, ind_label.split("/")[0], ind_label.split("/")[1])

        for plotted_quantity, label_plotted_quantity in zip(["<" + quantity + ">", "median_" + quantity],
                                                            ["mean_" + quantity, "median_" + quantity]):
            filename = "cont-discr_comp_" + quantity + "_" + label_plotted_quantity + ind_label.replace("/",
                                                                                                        "_") + ".pdf"
            my_title = write_title1(quantity, filename, dict_distribs[ind_label.split("/")[1]])
            my_prefix = ""
            df_aux1 = df1.loc[ind_label]
            df_aux2 = df2.loc[ind_label]
            x = df_aux1["N"].to_numpy()
            y0 = [ES_cont_whole_dataset for x_variable in x ]
            y1 = df_aux1[plotted_quantity].to_numpy()
            x2 = df_aux2["N"].to_numpy()
            y2 = df_aux2[plotted_quantity].to_numpy()
            np.testing.assert_array_equal(x, x2)
            li_y_to_plot = [y0, y2, y1]

            col1 = mcolors.CSS4_COLORS["navy"];
            col2 = mcolors.CSS4_COLORS["mediumblue"]  # DEV: See https://matplotlib.org/stable/gallery/color/named_colors.html
            my_ylim = [floor(250 * min(min(y1), min(y2))) / 250, ceil(250 * max(ES_cont_whole_dataset, max(y1), max(y2))) / 250]


            make_plots_2D(quantity, output_dir, filename, x, li_y_to_plot,
                          ["Fit to all data (continuous)","Fit to prob. density (continuous)","Historical Simulation (discrete)"],
                          ["gray",col1, col2], [ "-",  (5, (10, 3)),(0, (3, 1, 1, 1))], my_title, None, my_prefix, my_ylim, False,"center right", my_markershapes=["s","o","s","o"] )

            my_ylim = [floor(250 * min(y1[3], y2[3])) / 250, ceil(250 * max(max(y1), max(y2))) / 250]
            make_plots_2D(quantity, output_dir, filename.replace(".pdf", "-smaller.pdf"), x, li_y_to_plot,
                          [None,None, None], ["gray",col1, col2], [ "-",  (5, (10, 3)), (0, (3, 1, 1, 1))], None, None, my_prefix, my_ylim, True, my_markershapes=["s","o","s","o"] )
    ...
    ''' 
        ...
        color_fill = dict_my_colors[ind_label.split("/")[1] + "-" + quantity]
        make_plots_2D( quantity, output_dir, filename, x, li_y_to_plot, my_labels, [col1,col2,col2,col3], ["-", ":", ":", "-."], my_title, color_fill, prefix, ylim )
    ...
    for ind_label in li_plots:
        ...
        y3 = df1["CI_2"].to_numpy()
        li_y_to_plot = [y1, y2, y3 ]
        filename = prefix+"_median-" + ind_label.replace("/", "_") + ".pdf" #aaa
        color_fill = dict_my_colors[ind_label.split("/")[1] + "-" + quantity]
        col1 = mcolors.CSS4_COLORS["navy"]; col2 = mcolors.CSS4_COLORS["mediumblue"] # DEV: See https://matplotlib.org/stable/gallery/color/named_colors.html
        make_plots_2D( quantity, output_dir, filename, x, li_y_to_plot, ["Median", "95% confidence interval", None ],[col1, col2, col2], ["-", ":", ":" ], my_title, color_fill, prefix, ylim)

    del df1; del df2; del filename; del x; del li_y_to_plot
    '''
    return
