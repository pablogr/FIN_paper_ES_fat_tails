...
def find_initial_point( dataset_in, consider_skewness = False ):
    ...
    data_median = np.median(dataset_in)
    if (abs(data_median)>0.05 ):
        li_locations_sg = [0]
    else:
        li_locations_sg = [ np.median(dataset_in) ] #xx Check si esto es sensato. Observamos q con N=10, el fit no converge si la loc es alta
...
def fit_to_nct_global_minimum( dataset_in, n_random_tries=100, max_n_iter=150, consider_skewness=True, verbose=0 ):
    ...
    if (len(dataset_in <= 30)):   n_random_tries *= 4
    elif (len(dataset_in <= 50)): n_random_tries *= 2
    ...
    if (verbose>0):
        ...
        print(" The GLOBAL minimum (nct) is:", loc_param_opt,  sca_param_opt, skew_param_opt, df_param_opt , loss_opt,"\n")
