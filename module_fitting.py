...
def fit_to_distributions( input_params ):
    ''' This function fits data stored in datafiles to probability density functions.

    :param input_params:
    :param file_fit_path: (str)
    :return:
    '''

    from module_generic_functions import create_file_fitting_parameters

    if (input_params.calculation_mode=="ES_distrib-vs-N_last_dates" ): return

    file_fit_path = create_file_fitting_parameters(input_params)

    for product_name in input_params.list_product_labels[1]:
        print("\n *** Now analysing", product_name, "***")
        for pdf_type in input_params.list_distribution_types:
            my_fitted_ts = FittedTimeSeries(input_params, product_name, pdf_type, file_fit_path)
...
class FittedTimeSeries:
    ...
    def __init__( self, input_params, product_name, pdf_type, file_fit_path_or_array ):
        ''' Initialization (constructor) of the class.

        :param input_params: (class InputParams) set of information with parameters to run the calculations
        :param product_name: (string) name (e.g. ISIN) of the product to analyse
        :param pdf_type: (string) name (in scipy.stats module) of the function to be fitted (must be either "nct", "norm", "genhyperbolic" or "levy_stable").
        :param file_fit_path_or_array: (string or numpy array). If it is a string, then it is the path to the file which stores the
                                        parameters of the fitting to probability density functions. If it is a numpy array, then this is the
                                        time series to fit itself.
        '''
        # price_from_yield: Set to True if the raw time-series corresponds to yields (not to prices). The bond price will then be inferred from these yields.

        # Definition of parameters
        self.calc_fitting = input_params.calc_fitting
        self.input_directory = input_params.input_directory

        assert type(file_fit_path_or_array) in [str, type(np.array([]))]
        if ( type(file_fit_path_or_array) == type(np.array([])) ):
            self.input_from_array = True
            self.file_fit_path    = None
        else: # file_fit_path_or_array is indeed a path (string)
            self.input_from_array = False
            self.file_fit_path = file_fit_path_or_array
        self.calc_ES_distrib = input_params.calc_ES_distrib
        self.calc_ES_HS = input_params.calc_ES_HS
        self.calc_fitting = input_params.calc_fitting
        self.filename = product_name
        self.output_directory = input_params.output_directory
        self.directory_output_plots = input_params.directory_output_plots
        self.type_of_return = input_params.type_of_return
        self.pdf_type = pdf_type.lower()
        self.max_n_iter = input_params.max_n_iter
        self.max_n_iter_levy = input_params.max_n_iter_levy
        self.output_fit_dir = input_params.output_fit_dir
        self.n_random_trials_fit = input_params.n_random_trials_fit
        self.only_plots = input_params.only_plots
        self.make_plots = input_params.make_plots
        self.n_random_trials_fit_levy = input_params.n_random_trials_fit_levy
        self.verbose = input_params.verbose
        self.product_type = input_params.product_type
        assert input_params.type_of_return in ["absolute","relative","logarithmic"]
        if not ( pdf_type in ["norm", "nct", "genhyperbolic", "levy_stable"]): raise Exception("\n ERROR: The function type is "+str(pdf_type)+" while just norm, nct, genhyperbolic and levy_stable are allowed. Please, redefine your input data.\n")

        if (self.product_type == "stocks"):
            self.filepathin = path.join(self.input_directory, str("ret_"+product_name + ".csv"))
        else: #(self.product_type=="bonds"):
            self.filepathin = path.join(self.input_directory, str(product_name+".csv")  )  #f"{ self.input_directory }{product_name}.csv"
        ...
        self.bottom_limit = input_params.bottom_limit
        self.top_limit = input_params.top_limit
        #  self.truncation_limit = -70 # This was deprecated and changed by bottom_limit and top_limit
        self.alpha_for_ES = 0.025
        self.price_from_yield = input_params.price_from_yield # If True, we have to convert yields to prices. We do it with assuming zero-coupon, i.e. with Price = 1/(1+Yield)^TTM
        self.ttm = input_params.time_to_maturity
        self.field_to_read = input_params.field_to_read

        if (self.input_from_array):

            self.np_ret = np.sort(  file_fit_path_or_array )

        else:  # file_fit_path_or_array is indeed a path (string)

            # Calculation of returns

            if ( self.product_type == "stocks"):  # The return is read from file (because its calculation must take into account the reinvestment of dividends)

                ret_col_name = "price_" + str( self.field_to_read) + "_corrected_log_ret"  # e.g. "price_Close_corrected_log_ret"
                df_aux = pd.read_csv(path.join(self.input_directory, "ret_" + product_name + ".csv"),  usecols=["Date",ret_col_name])
                #print(ret_col_name, path.join(self.input_directory, "ret_" + product_name + ".csv")); print(df_aux)
                df_aux = df_aux.tail( len(df_aux)-1 )
                df_aux.set_index("Date", inplace=True)
                self.df_ts = df_aux.copy()  # "ts" stands for time-series
                self.np_ret = np.sort(self.df_ts[ret_col_name].to_numpy())

            else: # if ( self.product_type == "bonds"):  Calculation of returns of PRICE. This stores a df with Date as index, in datetime format, in chronological order. It also stores returns in a sorted np array.

                df0 = pd.read_csv(self.filepathin, usecols=["Date",self.field_to_read])  # IN: In a bond from investing.com, if field_to_read is "Price", then it is indeed a yield (that is what the name of the file states); moreover, we red elsewhere that it means the closing value.
                if (self.price_from_yield):
                    df0["price_from_yield"] = 100 /( (1+df0[self.field_to_read]/100)**self.ttm ) # By default, we price the bonds to 100.
                    field_to_get_returns = "price_from_yield"
                else:
                    field_to_get_returns = self.field_to_read

                # Conversion of the date
                df0 = self.convert_to_date( df0 )
                df0.sort_values(by='Date', inplace=True)
                df0.set_index("Date", inplace=True)

                if (self.type_of_return=="absolute"):
                    ret_col_name = 'abs_ret'
                    df0[ret_col_name] = df0[field_to_get_returns] - df0[field_to_get_returns].shift(1)
                elif (self.type_of_return=="relative"):
                    ret_col_name = "rel_ret"
                    df0[ret_col_name] = (df0[field_to_get_returns] - df0[field_to_get_returns].shift(1)) / df0[field_to_get_returns]
                elif (self.type_of_return=="logarithmic"):
                    ret_col_name = "log_ret"
                    df0[ret_col_name] = np.log( df0[field_to_get_returns]) - np.log( df0[field_to_get_returns].shift(1) )
                else:
                    raise Exception("\nERROR: Unknown return type: "+str(self.type_of_return)+". Please, check the code.\n")

                df0 = df0.iloc[1:, :]
                num_nans = df0[ret_col_name].isnull().sum()
                if (num_nans != 0): raise Exception("\nERROR: " + str(num_nans) + " NaNs found. Please, check your data.\n")
                self.df_ts = df0 # "ts" stands for time-series
                self.np_ret = np.sort( self.df_ts[ret_col_name].to_numpy() )

                # self.np_ret = self.df_ts[ret_col_name].to_numpy()  #  TRUNCAMOS PARA AGILIZAR! Borrar ESTO!!!
                # self.np_ret = self.np_ret[0:80]
                # self.np_ret = np.sort(self.np_ret )

        # End of "init"---------------------

    def convert_to_date(self,df_in):
        '''This function rewrites the dates of the time-series to avoid confusion between days and months.

        :param df_in: (DataFrame) A dataframe having one column with the date and other colum with the time series to analyse. '''

        sep_found = False
        for sep in [".","-","/"]:
            if  (sep in df_in.loc[0,"Date"]):
                sep_found=True
                break
        if not sep_found: raise Exception("\nERROR: Unable to find the separator of the date in "+str(df_in.loc[0,"Date"])+"; Please, check your data.\n")

        max1=-1;max2=-1;max3=-1
        for ind in df_in.index:
            one_date = df_in.loc[ind,"Date"]
            if (one_date.count(sep)!=2): raise Exception("\nERROR: The date "+str(one_date)+" does not have 2 separators ("+str(sep)+") as expected. Please, check your data.\n")
            max1 = max(max1, int(one_date.split(sep)[0]))
            max2 = max(max2, int(one_date.split(sep)[1]))
            max3 = max(max3, int(one_date.split(sep)[2]))

        if   ( (max1>2000) and (max2<13) and (max3>13) ):       format_str = '%Y' + sep + '%m' + sep + '%d'
        elif ( (max1 > 2000) and (max2 > 13) and (max3 < 13) ): format_str = '%Y' + sep + '%d' + sep + '%m'
        elif ((max1 > 13) and (max2 > 2000) and (max3 < 13)):   format_str = '%d' + sep + '%Y' + sep + '%m'
        elif ((max1 < 13) and (max2 > 2000) and (max3 > 13)):   format_str = '%m' + sep + '%Y' + sep + '%d'
        elif ((max1 < 13) and (max2 > 13) and (max3 > 2000)):   format_str = '%m' + sep + '%d' + sep + '%Y'
        elif ((max1 > 13) and (max2 < 13) and (max3 > 2000)):   format_str = '%d' + sep + '%m' + sep + '%Y'
        else: raise Exception("\nERROR: Could not properly identify day, month and year in the dates. Please, check your data.\n")

        for ind in df_in.index:
            df_in.loc[ind, "Date"] = (datetime.datetime.strptime( str(df_in.loc[ind, "Date"]), format_str)).date()

        return df_in


    # Fitting of the function
    def fit_to_distribution(self ):
        '''  This fits the data of returns to a given probability distribution.'''

        if ( (not self.input_from_array) and ((not (self.calc_fitting)) or (self.only_plots))):
            from module_generic_functions import read_params_from_file
            self.fitting_parameters = read_params_from_file(self.filename, self.pdf_type, self.type_of_return, self.output_fit_dir, self.only_plots, self.calc_fitting)
            if (self.only_plots): self.plot_fitting()
            return

        if (not self.input_from_array):

            if (self.pdf_type=="nct"):
                 print("\n * Now fitting the data of "+self.filename + " to a non-centered t-student distribution.\n")
            elif (self.pdf_type == "genhyperbolic"):
                if (self.consider_p):
                ...
                print("\n * Now fitting the data of " + self.filename + " to a normal (Gaussian) distribution.\n")
            elif (self.pdf_type == "levy_stable"):
                print("\n * Now fitting the data of " + self.filename + " to a Levy-stable distribution.\n")
        ...
        if (self.pdf_type == "nct"):
            ...
            self.fitting_parameters = fit_to_nct_global_minimum( self.np_ret, self.n_random_trials_fit, self.max_n_iter, self.consider_skewness, verbose )
        elif (self.pdf_type == "genhyperbolic"):
            from module_fitting_genhyperbolic import fit_to_genhyperbolic_global_minimum
            self.fitting_parameters = fit_to_genhyperbolic_global_minimum( self.np_ret, self.n_random_trials_fit, self.max_n_iter, self.consider_skewness, self.consider_p, verbose )
        elif (self.pdf_type == "norm"):
            from module_fitting import fit_to_normal
            self.fitting_parameters = fit_to_normal(  self.np_ret, verbose)
        elif (self.pdf_type == "levy_stable"):
            from module_fitting_stable import fit_to_stable_global_minimum
            from module_generic_functions import read_params_nct
            self.fitting_parameters = fit_to_stable_global_minimum( self.np_ret, self.n_random_trials_fit_levy, self.max_n_iter_levy, self.consider_skewness, verbose, read_params_nct( self.output_fit_dir, self.filename ) )
        else:
            raise Exception("\nERROR: Unrecognized probability density type "+str(self.pdf_type)+".\n")

        if not self.input_from_array:
            self.update_df_fitting_parameters()#( self.file_fit_path, self.type_of_return, self.only_plots, self.filename, self.pdf_type, self.fitting_parameters)
            self.plot_fitting( )


    def update_df_fitting_parameters(self):
        '''This function updates the dataframe where the parameters of the fitting to residuals will be stored.'''

        product_name = self.filename

        #file_fit_path, type_of_return, only_plots, , pdf_type, dict_fitting_parameters

        if ( self.only_plots): return

        type_of_return =  self.type_of_return

        df_fitting_params = pd.read_csv( self.file_fit_path, header=0)
        df_fitting_params = df_fitting_params.set_index("Product_name")
        ...
        distrib_type_df = dict_df_fitting[self.pdf_type]  # e.g. nct, normal

        df_fitting_params.loc[product_name, "ret_type"] = type_of_return
        df_fitting_params.loc[product_name, distrib_type_df + "_loc"]   = self.fitting_parameters['loc_param']
        df_fitting_params.loc[product_name, distrib_type_df + "_scale"] = self.fitting_parameters['scale_param']
        df_fitting_params.loc[product_name, distrib_type_df + "_loss"]  = self.fitting_parameters['loss']

        if (self.pdf_type == "nct"):
            df_fitting_params.loc[product_name, distrib_type_df + "_skparam"] = self.fitting_parameters['skewness_param']
            df_fitting_params.loc[product_name, distrib_type_df + "_dfparam"] = self.fitting_parameters['df_param']
        elif (self.pdf_type == "levy_stable"):
            df_fitting_params.loc[product_name, distrib_type_df + "_alpha_param"] = self.fitting_parameters['alpha_param']
            df_fitting_params.loc[product_name, distrib_type_df + "_beta_param"] = self.fitting_parameters['beta_param']
        elif (self.pdf_type == "genhyperbolic"):
            df_fitting_params.loc[product_name, distrib_type_df + "_a_param"] = self.fitting_parameters['a_param']
            df_fitting_params.loc[product_name, distrib_type_df + "_b_param"] = self.fitting_parameters['b_param']
            df_fitting_params.loc[product_name, distrib_type_df + "_p_param"] = self.fitting_parameters['p_param']

        df_fitting_params.to_csv(self.file_fit_path, index=True)
...
def fit_to_normal( ts_to_fit, verbose=0 ):
    ...
    del ts_to_fit
    if (verbose>0): print("The GLOBAL minimum (normal) is: ",normloc, normsca,"; Loss:",loss_norm )
...
def calc_ES_discrete(data_in, truncation_limit=-70.0, alpha=0.025):
...
def first_iteration(dataset_in, distrib_type, consider_skewness, loc_param0, sca_param0, skewness_param0, tail_param0, form_param0=None, consider_nonzero_p=False, verbose=0):
    ...
    if (distrib_type == "nct"):
        ...
        factor_accept = 0.95; cauchy_scale = 0.001;
        arr_unif = uniform(1,4,12)
        for (i,step) in zip(range(12), [ -10**(-7), 10**(-7), -10**(-8), 10**(-8),-3*10**(-8), 3*10**(-8), -3*10**(-7), 3*10**(-7), -10**(-6), 10**(-6), -10**(-5), 10**(-5) ]) :
            step *= arr_unif[i]
            ...
            loss1 = calc_loss_stable(dataset_in, loc_param1, sca_param1, skewness_param1, tail_param1 )
            if not (isnan(loss1)):break
    elif (distrib_type == "levy_stable"):
        from module_fitting_stable import calculate_gradient_params
        if (verbose > 0): print("\n * Now doing the first iteration.")
        ...
        loss0 = calc_loss_stable(dataset_in, loc_param0, sca_param0, skewness_param0, tail_param0 )
...
def first_iteration_single_param( sp, dataset_in, distrib_type, consider_skewness, lim_params, loc_param0, sca_param0, skewness_param0, tail_param0, form_param0=None):
    ...
    if (isnan(my_grad)):
        my_grad = 0.00001*uniform(-20, 20)
        print("WARNING: Gradient was NaN; it was given an arbitrary value (",my_grad,").")

    loss1=999; count = 0

    while (((loss1 > 0.995 * loss0) or (np.isinf(loss0)) or (isnan(loss0)) or (isnan(loss1))) and (count < 15)):

        count += 1
        step = cauchy.rvs(size=1, loc=0, scale=0.001)[0]
        ...
        if (distrib_type == "nct"):
            ...
            loss1 = calc_loss_stable(dataset_in, loc_param0, sca_param0, updp1['b'], updp1['a'] )
    ...
    if (count==50): # Unable to find an appropriate first step
        if not (distrib_type == "levy_stabe"):
            ...
            for step in [-10 ** (-8), 10 ** (-8), -3 * 10 ** (-8), 3 * 10 ** (-8), -10 ** (-7), 10 ** (-7), -3 * 10 ** (-7), 3 * 10 ** (-7), -10 ** (-6), 10 ** (-6), -10 ** (-5), 10 ** (-5)]:
                ...
                #loss1 = - (np.sum(np.log(levy_stable.pdf(dataset_in, loc=loc_param0, scale=sca_param0, beta=updp1['b'],alpha=updp1['a'])))) / len(dataset_in)
                loss1 = calc_loss_stable(dataset_in, loc_param0, sca_param0, updp1['b'], updp1['a'])
                if ((loss1 < 0.98 * loss0) and ( not isnan(loss1))):
                    break
            if( isnan(loss1)):
                return None, None, None
...
class Spread:
    ...
    def calc_spread(self):
        ...
        # eso habrá que revisarlo. Esto lo que da es exactamente lo q dice la explicación más arriba. XX
...
def calc_loss_stable( dataset_in, loc_param0, sca_param0, skewness_param0, tail_param0, thresh=1.0e-8):
    '''This function calculates the loss function of a levy_stable distribution. This subroutine is necessary because
    loss0 = - (np.sum(np.log(levy_stable.pdf(dataset_in, loc=loc_param0, scale=sca_param0, beta=skewness_param0,alpha=tail_param0)))) / len(dataset_in)
    gives problems for low values of the pdf.
    '''
    my_array = levy_stable.pdf(dataset_in, loc=loc_param0, scale=sca_param0, beta=skewness_param0, alpha=tail_param0)
    my_array = my_array[(my_array > thresh) | (my_array < -thresh)]
    loss0 = - (np.sum(np.log(my_array))) / len(my_array)
    del dataset_in; del my_array
